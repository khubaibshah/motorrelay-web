...
<template>
  <section class="space-y-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
    <header>
    ...
    <div class="space-y-2">
      <div
        ...
        :key="section.key"
        class="flex items-start justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2.5"
      >
        <div class="min-w-0 flex-1">
          <p class="text-[11px] font-black uppercase tracking-wide text-slate-400">{{ section.label }}</p>
          <div v-if="section.lines" class="mt-0.5 space-y-0.5 text-sm font-semibold leading-5 text-slate-950">
            <p v-for="line in section.lines" :key="line" class="break-words">{{ line }}</p>
          </div>
          <p v-else class="mt-0.5 truncate text-sm font-semibold leading-5 text-slate-950">{{ section.value }}</p>
        </div>
        <button
          type="button"
          class="shrink-0 rounded-full border border-slate-200 bg-white px-3 py-1 text-[11px] font-bold text-slate-600 hover:border-emerald-200 hover:text-emerald-700"
