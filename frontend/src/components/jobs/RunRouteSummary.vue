...
<template>
  <section class="tile space-y-1.5" :class="compact ? 'p-2.5' : 'p-4'">
    <p
      v-if="copiedMessage"
    ...
    <div class="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-stretch gap-1.5">
      <div class="rounded-xl bg-slate-50 dark:bg-white/[0.06]" :class="compact ? 'p-2' : 'p-2.5'">
        <div class="flex items-center justify-between gap-2">
          ...
          >
            <svg viewBox="0 0 24 24" class="size-3.5" fill="none" stroke="currentColor" stroke-width="1.9" aria-hidden="true">
              <rect x="9" y="9" width="10" height="10" rx="2" />
              <path stroke-linecap="round" stroke-linejoin="round" d="M6 15H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v1" />
            </svg>
          </button>
        </div>
        <p class="mt-0.5 break-words font-black leading-tight text-slate-950 dark:text-white" :class="compact ? 'text-sm' : 'text-base'">{{ pickupTitle }}</p>
        <p v-if="showPickupPostcode" class="mt-0.5 text-[0.7rem] text-slate-600 dark:text-emerald-100">
          {{ job.pickup_postcode || '--' }}
        </p>
        <p v-if="pickupScheduleLabel" class="mt-1 text-[0.62rem] font-black uppercase tracking-wide text-emerald-700 dark:text-emerald-300">
      ...
      <div class="rounded-xl bg-slate-50 dark:bg-white/[0.06]" :class="compact ? 'p-2' : 'p-2.5'">
        ...
        <p class="mt-0.5 break-words font-black leading-tight text-slate-950 dark:text-white" :class="compact ? 'text-sm' : 'text-base'">{{ dropoffTitle }}</p>
        <p v-if="showDropoffPostcode" class="mt-0.5 text-[0.7rem] text-slate-600 dark:text-emerald-100">
          {{ job.dropoff_postcode || '--' }}
        </p>
        <p v-if="deliveryScheduleLabel" class="mt-1 text-[0.62rem] font-black uppercase tracking-wide text-emerald-700 dark:text-emerald-300">
    ...
    <div class="grid grid-cols-2 gap-1.5 text-sm">
      <div class="rounded-xl bg-slate-50 dark:bg-white/[0.06]" :class="compact ? 'p-2' : 'p-2.5'">
        <p class="text-[0.65rem] font-black uppercase tracking-wide text-slate-500 dark:text-emerald-100">Distance</p>
        <p class="text-sm font-black leading-tight text-slate-950 dark:text-white">{{ distanceLabel }}</p>
      </div>

      <div class="rounded-xl bg-slate-50 dark:bg-white/[0.06]" :class="compact ? 'p-2' : 'p-2.5'">
        <p class="text-[0.65rem] font-black uppercase tracking-wide text-slate-500 dark:text-emerald-100">Transport</p>
        <p class="text-sm font-black leading-tight text-slate-950 dark:text-white">{{ transportLabel }}</p>
      </div>
    </div>
  </section>
</template>
