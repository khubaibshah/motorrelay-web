  ...
  <div class="grid gap-4 lg:grid-cols-[2fr_1fr]">
    <div class="space-y-4">
      ...
      <RouterLink
        v-if="isDriver"
        to="/profile/identity"
        class="block rounded-2xl border border-emerald-200 bg-emerald-50 p-5 transition hover:border-emerald-300 hover:bg-emerald-100 dark:border-emerald-400/30 dark:bg-emerald-400/10 dark:hover:bg-emerald-400/15"
      >
        <div class="flex items-center justify-between gap-4">
          <div class="min-w-0">
            <p class="text-xs font-black uppercase tracking-wide text-emerald-700 dark:text-emerald-300">Driver verification</p>
            <h2 class="mt-1 text-lg font-black text-emerald-950 dark:text-white">Identity &amp; payout setup</h2>
            <p class="mt-1 text-sm leading-6 text-emerald-800 dark:text-emerald-100">Manage Stripe verification, licence details, insurance and trade plates in one place.</p>
          </div>
          <span class="shrink-0 text-sm font-bold text-emerald-700 dark:text-emerald-300">Open →</span>
        </div>
      </RouterLink>
